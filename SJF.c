...
Shortest - It is an primtive approach to decide how cpu will execute processes on the basis 
of thier burst time
This program is a semi primtive solution to the problem. as it will solve everything non-primtively solely
on the basis of there burst time but if there is a job which starts and finishes before burst time of shortest
burst time job it will get executed.

step-1 - Store all the arival time in an array
step-2 - store all the burst time in an another array

In a loop
step-3 -find the index of the shortest elemt in burst time array
step-4 -if there is some process which starts and ends faster than the min burst execute it
...
int main() {
    ...
    for(x=0;x<nj;x++) {
        min=0;
        for(y=0;y<nj;y++){
            if(burstT[y]<burstT[min]){
               min=y;
            }
        }
        for(y=0;y<nj;y++) {
        if(arivatT[min]>(arivatT[y]+burstT[y])){
            min = y;
        } 
        }
